...
func TestUserLifecycle_FullFlow(t *testing.T) {
	...
	// Poll for audit logs with retry mechanism to handle async processing
	var userLogs []auditModel.AuditLogResponse
	deadline := time.Now().Add(5 * time.Second)

	for time.Now().Before(deadline) {
		// Manually trigger outbox sync repeatedly since the worker might be slow or interval-based
		...
		if err == nil {
			...
			// If we have enough logs, we can stop waiting
			if len(userLogs) >= 4 {
				break
			}
		}
	...
	require.GreaterOrEqual(t, len(userLogs), 4, "Should have at least 4 audit entries for this lifecycle after polling")
