...
func TestUserLifecycle_FullFlow(t *testing.T) {
	...
	authUC := authUseCase.NewAuthUsecase(5, 30*time.Minute, 3, jwtManager, tokenRepo, userRepo, oRepo, tm, env.Logger, nil, authz, taskDistributor, nil, make(map[string]sso.Provider), "http://localhost:3000")
