...
func TestUserLifecycle_FullFlow(t *testing.T) {
	...
	// Polling mechanism to wait for async audit logs (especially LOGIN which goes through Redis worker)
	var userLogs []auditModel.AuditLogResponse
	deadline := time.Now().Add(5 * time.Second)

	for time.Now().Before(deadline) {
		// Manually trigger outbox sync repeatedly as some logs might be in outbox
	...
	require.GreaterOrEqual(t, len(userLogs), 4, "Should have at least 4 audit entries for this lifecycle (found: %d)", len(userLogs))
