	...
	tusHandler, err := handler.NewHandler(handler.Config{
		...
		NotifyCompleteUploads: true,
			PreUploadCreateCallback: func(hook handler.HookEvent) (handler.HTTPResponse, handler.FileInfoChanges, error) {
				return defaultPreUploadCreateCallback(hook, registry)
			},
	})
	if err != nil {
		return nil, err
...
func defaultPreUploadCreateCallback(hook handler.HookEvent, registry *Registry) (handler.HTTPResponse, handler.FileInfoChanges, error) {
