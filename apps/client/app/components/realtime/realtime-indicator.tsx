...
export function RealtimeIndicator({ showLabel, className }: RealtimeIndicatorProps) {
  ...
  const color =
    status === "connected" ? "text-success" : status === "partial" ? "text-warning" : "text-danger";
  const dotColor =
    status === "connected" ? "bg-success" : status === "partial" ? "bg-warning" : "bg-danger";
        ...
        <span className={cn("inline-flex cursor-default items-center gap-1.5", className)}>
          <span className="relative flex h-2 w-2">
            {(allConnected || partialConnected) && (
              <span
                className={cn("absolute inset-0 animate-ping rounded-full opacity-75", dotColor)}
              />
            )}
