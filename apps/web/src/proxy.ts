...
export function proxy(request: NextRequest) {
  ...
  const detectedLocale = localeMatch && locales.includes(localeMatch[1]) ? localeMatch[1] : null;
  const localePrefix = detectedLocale ? `/${detectedLocale}` : "";

  // 1. Protect /dashboard routes
  const isDashboardPath =
    pathname.startsWith(`${localePrefix}/dashboard`) || pathname.startsWith("/dashboard");
  const isAuthPath =
