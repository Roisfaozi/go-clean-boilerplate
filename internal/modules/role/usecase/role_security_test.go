...
func setupSecurityRoleTest(t *testing.T) (*mocks.MockRoleRepository, *permissionMocks.MockIPermissionUseCase, usecase.RoleUseCase) {
	mockRepo := mocks.NewMockRoleRepository(t)
	mockPerm := permissionMocks.NewMockIPermissionUseCase(t)
	logger := logrus.New()
	mockTM := mocking.NewMockWithTransactionManager(t)

	mockTM.On("WithinTransaction", mock.Anything, mock.Anything).Maybe().Return(func(ctx context.Context, fn func(context.Context) error) error {
		return fn(ctx)
	})
...
func TestDeleteRole_SuperadminProtection(t *testing.T) {
	repo, _, uc := setupSecurityRoleTest(t)
...
func TestDeleteRole_SuperadminProtection_CaseSensitive(t *testing.T) {
	repo, perm, uc := setupSecurityRoleTest(t)
