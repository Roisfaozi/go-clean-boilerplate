...
func TestRoleHandler_Create_XSS_Name(t *testing.T) {
	...
	// Since we now sanitize before validation, the <script> tags are stripped.
	// "alert(1)" remains, which is valid.
	expectedReq := &model.CreateRoleRequest{Name: "alert(1)", Description: "XSS role"}
	mockUseCase.On("Create", mock.Anything, expectedReq).Return(&model.RoleResponse{ID: "uuid", Name: "alert(1)"}, nil)
