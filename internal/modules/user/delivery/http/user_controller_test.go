...
func newTestUserHandler(mockUseCase *mocks.MockUserUseCase) *UserController {
	log := logrus.New()
	log.SetLevel(logrus.PanicLevel)
	validate := validator.New()
	_ = validation.RegisterCustomValidations(validate)
	return NewUserController(mockUseCase, log, validate)
}
...
func TestUserHandler_GetUsersDynamic(t *testing.T) {
	...
	t.Run("Validation Error - Invalid Sort", func(t *testing.T) {
		...
		jsonBody := `{"sort":[{"colId":"username","sort":"INVALID"}]}`
		req, _ := http.NewRequest(http.MethodPost, "/users/search", bytes.NewBufferString(jsonBody))
		req.Header.Set("Content-Type", "application/json")
