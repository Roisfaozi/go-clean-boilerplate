...
func TestOrganizationMemberUseCase_AcceptInvitation(t *testing.T) {
	...
	t.Run("Success - Sanitization", func(t *testing.T) {
		deps, uc := setupMemberTest()
		ctx := context.Background()
		xssName := "<script>alert(1)</script>Name"
		sanitizedName := "&lt;script&gt;alert(1)&lt;/script&gt;Name"
		req := &model.AcceptInvitationRequest{Token: "token", Password: "pass", Name: xssName}
		inv := &entity.InvitationToken{ID: "inv-1", Email: "new@example.com", OrganizationID: "org-1", Role: "role:member", ExpiresAt: time.Now().Add(1 * time.Hour).UnixMilli()}
		user := &userEntity.User{ID: "user-1", Email: "new@example.com", Status: "invited"}
	...
	t.Run("Success - Active User", func(t *testing.T) {
		deps, uc := setupMemberTest()
		ctx := context.Background()
		req := &model.AcceptInvitationRequest{Token: "token"} // No password needed
		inv := &entity.InvitationToken{ID: "inv-1", Email: "existing@example.com", OrganizationID: "org-1", Role: "role:member", ExpiresAt: time.Now().Add(1 * time.Hour).UnixMilli()}
		user := &userEntity.User{ID: "user-1", Email: "existing@example.com", Status: "active"}
		...
		deps.InvitationRepo.On("FindByToken", ctx, req.Token).Return(inv, nil)
		deps.UserRepo.On("FindByEmail", ctx, inv.Email).Return(user, nil)
		// No UserRepo.Update expected

		// Member doesn't exist yet
		deps.MemberRepo.On("GetMemberStatus", ctx, inv.OrganizationID, user.ID).Return("", nil)
		deps.MemberRepo.On("AddMember", ctx, mock.MatchedBy(func(m *entity.OrganizationMember) bool {
			return m.UserID == user.ID && m.OrganizationID == inv.OrganizationID && m.Status == entity.MemberStatusActive
		})).Return(nil)
