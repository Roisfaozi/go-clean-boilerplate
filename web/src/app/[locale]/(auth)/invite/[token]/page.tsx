        ...
        <div className="mx-auto w-full max-w-sm lg:mx-0">
          ...
          <div className="mb-8">
            ...
            <p className="text-muted-foreground mt-2 text-sm">
              You&apos;ve been invited to join an organization. Complete your details below.
            </p>
          </div>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div className="space-y-4 pt-4 border-t">
                <p className="text-[10px] font-bold uppercase text-muted-foreground tracking-widest">Set Security Credentials</p>
                <FormField
                    control={form.control}
                    name="password"
                    render={({ field }) => (
              ...
              <Button type="submit" className="w-full mt-6" disabled={isLoading}>
                {isLoading ? <Icon name="Loader" className="mr-2 h-4 w-4 animate-spin" /> : <Icon name="Check" className="mr-2 h-4 w-4" />}
                Join Organization
              </Button>
            </form>
          </Form>
      ...
      <div className="bg-primary relative hidden w-full flex-1 items-center justify-center overflow-hidden lg:flex">
        ...
        <div className="relative z-10 p-12 text-white text-center">
          <div className="max-w-md mx-auto">
            <div className="mb-8 flex h-16 w-16 mx-auto items-center justify-center rounded-2xl bg-white/20 backdrop-blur-md shadow-xl">
            ...
            <p className="mb-10 text-lg text-indigo-100">
              Welcome to the team! NexusOS handles the complexity of access control so you can focus on building what matters.
            </p>

            <div className="flex items-center justify-center gap-4 py-6 border-y border-white/10">
                <div className="flex -space-x-2">
                    {[1,2,3,4].map(i => (
                        <div key={i} className="h-8 w-8 rounded-full border-2 border-indigo-600 bg-indigo-200 flex items-center justify-center text-[10px] text-indigo-800 font-bold">
                            U{i}
                        </div>
                    ))}
                </div>
                <p className="text-sm text-indigo-100 font-medium">Join 10+ other team members</p>
            </div>
          </div>
        </div>
